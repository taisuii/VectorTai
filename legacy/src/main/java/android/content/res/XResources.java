...
import static com.android.bridge.TsHelpers.decrementMethodDepth;
import static com.android.bridge.TsHelpers.findAndHookMethod;
import static com.android.bridge.TsHelpers.getBooleanField;
import static com.android.bridge.TsHelpers.getLongField;
import static com.android.bridge.TsHelpers.getObjectField;
import static com.android.bridge.TsHelpers.incrementMethodDepth;
...
import com.android.bridge.TsMethodHook.MethodHookParam;
...
public class XResources extends XResourcesSuperClass {
	...
	private static final SparseArray<HashMap<String, CopyOnWriteSortedSet<TsLayoutInflated>>> sLayoutCallbacks = new SparseArray<>();
...
//			Log.w(TsBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	...
	public static void init(ThreadLocal<Object> latestResKey) throws Exception {
		...
		findAndHookMethod(LayoutInflater.class, "inflate", XmlPullParser.class, ViewGroup.class, boolean.class, new TsMethodHook() {
			@Override
		...
		final TsMethodHook parseIncludeHook = new TsMethodHook() {
			@Override
	 ...
	 * {@link IZygoteInitHook#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { TsBridge.log(t); }
				} else if (replacement instanceof Integer) {
	...
	public XmlResourceParser getLayout(int id) throws NotFoundException {
		...
		HashMap<String, CopyOnWriteSortedSet<TsLayoutInflated>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
		}
		if (inner != null) {
			CopyOnWriteSortedSet<TsLayoutInflated> callbacks;
			synchronized (inner) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						TsBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					TsBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				TsBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				TsBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			TsBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
...
//		}
	...
	private class XMLInstanceDetails {
		...
		public final CopyOnWriteSortedSet<TsLayoutInflated> callbacks;
		public final XResources res = XResources.this;

		private XMLInstanceDetails(ResourceNames resNames, String variant, CopyOnWriteSortedSet<TsLayoutInflated> callbacks) {
	 ...
	 */
	public TsLayoutInflated.Unhook hookLayout(int id, TsLayoutInflated callback) {
	 ...
	 * Hook the inflation of a layout.
	 *
	 * @deprecated Use {@link #hookLayout(String, String, String, TsLayoutInflated)} instead.
	 *
	 ...
	 */
	@Deprecated
	public TsLayoutInflated.Unhook hookLayout(String fullName, TsLayoutInflated callback) {
		int id = getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public TsLayoutInflated.Unhook hookLayout(String pkg, String type, String name, TsLayoutInflated callback) {
		int id = getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookLayout(id, callback);
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, TsLayoutInflated)}.
	 *
	 * @param id The ID of the resource which should be replaced.
	 * @param callback The callback to be executed when the layout has been inflated.
	 * @return An object which can be used to remove the callback again.
	 */
	public static TsLayoutInflated.Unhook hookSystemWideLayout(int id, TsLayoutInflated callback) {
		if (id >= 0x7f000000)
			throw new IllegalArgumentException("ids >= 0x7f000000 are app specific and cannot be set for the framework");
	 ...
	 * See {@link #hookSystemWideLayout(String, String, String, TsLayoutInflated)}.
	 *
	 * @deprecated Use {@link #hookSystemWideLayout(String, String, String, TsLayoutInflated)} instead.
	 *
	 ...
	 */
	@Deprecated
	public static TsLayoutInflated.Unhook hookSystemWideLayout(String fullName, TsLayoutInflated callback) {
		int id = getSystem().getIdentifier(fullName, null, null);
		if (id == 0)
			throw new NotFoundException(fullName);
	 ...
	 * @return An object which can be used to remove the callback again.
	 */
	public static TsLayoutInflated.Unhook hookSystemWideLayout(String pkg, String type, String name, TsLayoutInflated callback) {
		int id = getSystem().getIdentifier(name, type, pkg);
		if (id == 0)
			throw new NotFoundException(pkg + ":" + type + "/" + name);
		return hookSystemWideLayout(id, callback);
	...
	private static TsLayoutInflated.Unhook hookLayoutInternal(String resDir, int id, ResourceNames resNames, TsLayoutInflated callback) {
		if (id == 0)
			throw new IllegalArgumentException("id 0 is not an allowed resource identifier");
	...
	/** @hide */
	public static void unhookLayout(String resDir, int id, TsLayoutInflated callback) {
		HashMap<String, CopyOnWriteSortedSet<TsLayoutInflated>> inner;
		synchronized (sLayoutCallbacks) {
			inner = sLayoutCallbacks.get(id);
			if (inner == null)
